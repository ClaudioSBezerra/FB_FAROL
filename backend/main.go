...
// FB_FAROL — Sistema de Recalibração de Picking
// Version: 1.0.1
import (
	"context"
	"database/sql"
...
func initDBAsync() {
	go func() {
		...
		if connStr == "" {
			connStr = "postgres://postgres:postgres@localhost:5432/fb_farol?sslmode=disable"
...
func main() {
	_ = godotenv.Load()
	fmt.Printf("FB_FAROL Backend v%s\n", BackendVersion)
	...
	http.HandleFunc("/api/health", func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(HealthResponse{
			...
			Service:   "FB_FAROL Recalibração de Picking",
	...
	http.HandleFunc("/api/farol/produtos-faturados", func(w http.ResponseWriter, r *http.Request) {
		database := getDB()
		if database == nil {
			http.Error(w, "Database initializing...", http.StatusServiceUnavailable)
			return
		}
		handlers.FarolAPIKeyAuth(handlers.ProdutosFaturadosAPIHandler)(database)(w, r)
	})
	http.HandleFunc("/api/farol/sazonalidade-secao", func(w http.ResponseWriter, r *http.Request) {
		database := getDB()
		if database == nil {
			http.Error(w, "Database initializing...", http.StatusServiceUnavailable)
			return
	...
	// ── Farol — Dashboard de Urgência e Propostas (Epic 5) ───────────────
	...
	fmt.Printf("FB_FAROL starting on port %s...\n", port)
