-- 166_upsert_aggs_mes_tempfast.sql
-- Otimização: usar TEMP TABLE com índice em vez de varrer vendas_faturadas 17×.
--
-- ANTES (mig 165): 17 INSERTs × full index scan em vendas_faturadas = 4-5 min/mês.
-- AGORA: 1 SELECT do mês para TEMP TABLE + 17 INSERTs em TEMP TABLE = ~30s/mês.
--
-- Estratégia:
--   1. CREATE TEMP TABLE _v_fat ON COMMIT DROP AS SELECT ... WHERE empresa/data
--   2. Criar índice no que precisar (cod_*, cnpj)
--   3. ANALYZE pro planner saber
--   4. Todos INSERTs leem de _v_fat (sequential scan rápido)
--   5. Idem _v_trans para vendas_transmitidas
--   6. SET LOCAL work_mem='256MB' para acelerar GROUP BY/DISTINCT
--
-- IDEMPOTENTE: CREATE OR REPLACE FUNCTION substitui a anterior.
...
BEGIN
    -- Memória extra para agregações deste statement só.
    ...
    SELECT
        ...
        SUM(v.pvenda), SUM(v.plucro), SUM(v.qt)
    FROM _v_fat v
    WHERE v.cod_fornec <> '' AND v.cod_gerente <> '' AND v.cod_supervisor <> '' AND v.cod_rca <> '' AND v.cnpj <> ''
    ...
    SELECT
        ...
        SUM(v.pvenda), SUM(v.plucro), SUM(v.qt)
    FROM _v_fat v
    WHERE v.cod_supervisor <> '' AND v.cod_rca <> '' AND v.cod_fornec <> ''
    ...
    SELECT
        ...
        SUM(v.pvenda), SUM(v.plucro), SUM(v.qt)
    FROM _v_fat v
    WHERE v.cod_supervisor <> '' AND v.cod_rca <> '' AND v.cod_fornec <> '' AND v.cnpj <> ''
    ...
    SELECT
        ...
        SUM(v.pvenda), SUM(v.plucro), SUM(v.qt)
    FROM _v_fat v
    WHERE v.cod_rca <> '' AND v.cod_fornec <> ''
    ...
    SELECT
        ...
        SUM(v.pvenda), SUM(v.plucro), SUM(v.qt)
    FROM _v_fat v
    WHERE v.cod_rca <> '' AND v.cod_fornec <> '' AND v.cnpj <> ''
    ...
    SELECT v.empresa_id, p_ano, p_mes, v.cod_fornec, v.cod_gerente, v.cod_supervisor, v.cod_rca,
        ...
        COUNT(DISTINCT v.cod_prod) FILTER (WHERE v.qt > 0 AND v.cod_prod <> '')::NUMERIC,
        SUM(v.pvenda), SUM(v.plucro), SUM(v.qt)
    FROM _v_trans v WHERE v.cod_fornec <> '' AND v.cod_gerente <> '' AND v.cod_supervisor <> '' AND v.cod_rca <> '' AND v.cnpj <> ''
    GROUP BY v.empresa_id, v.cod_fornec, v.cod_gerente, v.cod_supervisor, v.cod_rca, v.cnpj
    ON CONFLICT (ano, empresa_id, mes, cod_fornec, cod_gerente, cod_supervisor, cod_rca, cnpj) DO UPDATE SET
