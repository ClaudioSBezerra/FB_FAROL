-- 170_fix_upsert_aggs_mes_call_v05.sql
-- ════════════════════════════════════════════════════════════════════════════
-- Bugfix da migration 169.
--
-- Sintoma: no painel mobile /m/CNPJ/sup/COD, ao clicar em "Por Fornecedor"
-- (view=V05) o backend retorna 0 cards mesmo havendo dados em vendas_faturadas
-- para aquele supervisor. Outras views (V01/V02/V03) funcionam.
--
-- Causa: a migration 167 transformou upsert_aggs_mes em um wrapper que chamava
--   PERFORM upsert_aggs_mes_core(...)   -- lógica V01-V04 (renomeada da 166)
--   PERFORM upsert_aggs_mes_v05(...)    -- lógica V05 separada
--
-- A migration 169 fez CREATE OR REPLACE de upsert_aggs_mes para implementar a
-- semântica rolling 12M (base_cli V01) e, ao fazê-lo, perdeu a chamada para
-- upsert_aggs_mes_v05. Resultado: nenhum mês foi populado em agg_*_v05_l*_mes
-- desde o deploy da 169.
--
-- Fix: recria a função upsert_aggs_mes (cópia 1:1 da 169) e ADICIONA no final
-- a chamada PERFORM farol.upsert_aggs_mes_v05(p_empresa_id, p_ano, p_mes).
-- Re-popula todos os meses ao final para preencher agg_*_v05_l*_mes.
--
-- Esta migration NÃO altera lógica de cálculo nem semântica — apenas restaura
-- a chamada V05 que foi perdida no rename/replace da 169.
-- ════════════════════════════════════════════════════════════════════════════

-- ── Original (preservado integralmente abaixo, sem mudanças no corpo) ────────
