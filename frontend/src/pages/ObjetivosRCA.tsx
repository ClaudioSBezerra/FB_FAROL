...
export default function ObjetivosRCA() {
  ...
  // Seleciona o primeiro período disponível automaticamente
  useEffect(() => {
  ...
  const { data: rows = [], isFetching } = useQuery<RCARow[]>({
    queryKey: ['objetivos-rca', periodoKey, search],
    queryFn: () => {
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <div className="space-y-1.5 flex-1 min-w-48 max-w-xs">
          ...
          <div className="relative">
            ...
            <Input
              className="pl-8"
              placeholder="RCA, supervisor ou fornecedor..."
              value={search}
            ...
            <TableBody>
              ...
              {rows.map((row, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell className="text-xs">
                    <span className="text-muted-foreground">{row.cod_rca} </span>
                    {row.nome_rca}
                  </TableCell>
